/**
 * This file is devived from the libopencm3 project.
 *
...
void sys_tick_handler(void)
{
	...
	unsigned char move = rotary_process(&encoder);
	if (move == DIR_CW) {
		// time = time << 1;
		if (++state >=4)
			state=0;
	} else if (move == DIR_CCW) {
		// if (time > 1)
		// 	time = time >> 1;
		if (--state < 0)
			state=3;
...
static void gpio_setup(void)
{
	...
	gpio_mode_setup(GPIOC, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO8 | GPIO0 | GPIO1 | GPIO2 | GPIO3);
}
...
int main(void)
{
	...
	encoder = rotary_init(GPIOA, GPIO0, GPIO2);
